...
func (d *DB) ListPackOwnership(collectionID int) ([]model.PackOwnership, error) {
	...
	for rows.Next() {
		...
		var owned int
		if err := rows.Scan(&po.Pack.Code, &po.Pack.Name, &po.Pack.CgdbID, &po.Pack.OctgnID,
			&po.Pack.DateRelease, &po.Pack.PackTypeCode, &po.Pack.Position, &po.Pack.Size, &owned); err != nil {
			return nil, err
		}
		po.Owned = owned == 1
...
func (d *DB) DropAll() error {
	for _, t := range []string{"collection_packs", "collections", "cards", "sets", "packs"} {
